...
/**
 * Test assignment on a 4x4 matrix. Example taken from http://www.ee.oulu.fi/~mpa/matreng/eem1_2-1.htm with kCost[0][1]
...
public class LinearAssignmentAPI {
    private static void runAssignmentOn4x4Matrix() {
        ...
        final int[][] cost = {{90, 76, 75, 80}, {35, 85, 55, 65}, {125, 95, 90, 105}, {45, 110, 95, 115}};
        ...
        if (assignment.solve() == LinearSumAssignment.Status.OPTIMAL) {
            ...
            for (int node = 0; node < assignment.getNumNodes(); ++node) {
                System.out.println("Left node " + node + " assigned to right node " + assignment.getRightMate(node)
