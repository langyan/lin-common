...
/**
 * Sample showing how to model and solve a capacitated vehicle routing problem with time windows using the swig-wrapped
 * version of the vehicle routing library in src/constraint_solver.
 */
public class CapacitatedVehicleRoutingProblemWithTimeWindows {
    private static Logger logger = Logger.getLogger(CapacitatedVehicleRoutingProblemWithTimeWindows.class.getName());
        ...
        return new LongBinaryOperator() {
            ...
            public long applyAsLong(long firstIndex, long secondIndex) {
                try {
                    ...
                    return (long)costCoefficient * (Math.abs(firstLocation.first - secondLocation.first)
    ...
    /**
     * Creates order data. Location of the order is random, as well as its demand (quantity), time window and penalty.
     *
    ...
    /**
     * Creates fleet data. Vehicle starting and ending locations are random, as well as vehicle costs per distance unit.
     *
     ...
     */
    private void buildFleet(int numberOfVehicles, int xMax, int yMax, int endTime, int capacity,
    ...
    private void solve(final int numberOfOrders, final int numberOfVehicles) {
        ...
        // Setting up orders
        for (int order = 0; order < numberOfOrders; ++order) {
            timeDimension.cumulVar(order).setRange(orderTimeWindows.get(order).first,
        ...
        // Solving
        RoutingSearchParameters parameters = main.defaultRoutingSearchParameters().toBuilder()
            .setFirstSolutionStrategy(FirstSolutionStrategy.Value.ALL_UNPERFORMED).build();
