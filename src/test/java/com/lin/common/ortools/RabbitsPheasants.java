...
public class RabbitsPheasants {
    ...
    /**
     * Solves the rabbits + pheasants problem. We are seing 20 heads and 56 legs. How many rabbits and how many
     * pheasants are we thus seeing?
     */
    private static void solve(boolean traceSearch) {
        ...
        solver.addConstraint(
            solver.makeEquality(solver.makeSum(solver.makeProd(rabbits, 4), solver.makeProd(pheasants, 2)), 56));
        DecisionBuilder db = solver.makePhase(rabbits, pheasants, Solver.CHOOSE_FIRST_UNBOUND, Solver.ASSIGN_MIN_VALUE);
        solver.newSearch(db);
        solver.nextSolution();
